...
export async function getUsers(token) {
	const res = await fetchWithTimeout(`${BASE_URL}/user/list`, { headers: authHeaders(token) });
...
export async function updateUserRole(id, role, token) {
	const res = await fetchWithTimeout(`${BASE_URL}/user/update-role/${id}`, {
		method: "PATCH",
...
export async function blockUser(id, token) {
	const res = await fetchWithTimeout(`${BASE_URL}/user/block/${id}`, { method: "PATCH", headers: authHeaders(token) });
	if (!res.ok) throw new Error(`Erro ao bloquear utilizador (${res.status}).`);
}

export async function unblockUser(id, token) {
	const res = await fetchWithTimeout(`${BASE_URL}/user/unblock/${id}`, { method: "PATCH", headers: authHeaders(token) });
	if (!res.ok) throw new Error(`Erro ao desbloquear utilizador (${res.status}).`);
}

export async function deleteUser(id, token) {
	const res = await fetchWithTimeout(`${BASE_URL}/user/delete/${id}`, { method: "DELETE", headers: authHeaders(token) });
